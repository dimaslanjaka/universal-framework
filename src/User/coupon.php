...
class coupon extends user
{
    ...
    public function create($coupon, $limit)
    {
        $this->pdo_required();
    ...
    public function pdo_required()
    {
        if (!$this->pdo_instance() || empty($this->pdo_instance())) {
            if ('any' == \MVC\helper::HeaderAccept() && !\MVC\helper::cors()) {
                throw new Exception('PDO Required', 1);
            } else {
    ...
    public function coupon_login($coupon)
    {
        $this->pdo_required();
     ...
     * @return string
     */
    public function gen_token($length = 10)
    {
        $token = '';
    ...
    public function coupon_data($key)
    {
    ...
    public function set_admin($data, $expire = 15, $cookie_path)
    {
        helper::mins(str_rot13('coupon_admin'), $data, $expire, $cookie_path, $_SERVER['HTTP_HOST']);
    }
    ...
    {
        return helper::has(str_rot13('coupon_admin'), false);
    }
     ...
     * @return bool
     */
    public function is_login($token = true)
    {
        if ($token) {
