...
class helper
{
    ...
    public static function require_method($method)
    {
     ...
     * @requires shell_exec
     */
    public static function sass($path)
    {
     ...
     * @return string
     */
    public static function fixSlash($path, $maxlength = 10)
    {
     ...
     * @return string|null
     */
    public static function php_error_log($onlypath = false)
    {
        $log = PHP_ERROR_FILE;
        if ($log = realpath($log)) {
     ...
     * @return string|null
     */
    public static function is_header($any)
    {
    ...
    public static function babel($path)
    {
    ...
    public static function webkit_asset($path, $alternative = null)
    {
        $path = realpath($path);
        if (!$path && $alternative) {
     ...
     */
    public static function base_url($path, $forceSSL = false)
    {
     ...
     * @return void
     */
    public static function get_canonical($url = null, $whos = true)
    {
        if (null === $url) {
            $url = self::geturl();
        }
     ...
     * @return string
     */
    public static function get_clean_uri($url = null)
    {
        if (null === $url) {
            $url = self::geturl();
        }
     ...
     * @param bool $httponly
     */
    public static function setcookie($name, $value = true, $day, $path = '/', $domain = '', $secure = false, $httponly = false)
    {
        settype($day, 'integer');
        if (empty($domain)) {
     ...
     * @return string|null
     */
    public static function request_data($type = 'get', $name)
    {
     ...
     * @return bool
     */
    public static function isLocal($regex = null)
    {
     ...
     */
    public static function get_url_path($path, $cache = null)
    {
        $load = function ($path) use ($cache) {
