...
async function createApp(withoutView) {
  ...
  await typescriptCompiler(
    upath.normalizeSafe(__dirname + "/tsconfig.build.json"),
    "./"
  ).catch(function (err) {
    log.log(log.error(err));
  });
  await typescriptCompiler(
    upath.normalizeSafe(__dirname + "/tsconfig.precompiler.json"),
    "./"
  ).catch(function (err) {
    log.log(log.error(err));
  });
  await typescriptCompiler(
    upath.normalizeSafe(__dirname + "/tsconfig.compiler.json"),
    "./libs/"
  ).catch(function (err) {
    log.log(log.error(err));
  });
...
function typescriptCompiler(source, destination, callback) {
  ...
  return new Promise((resolve, reject) => {
    exec(`tsc -p ${source}`, function (err, stdout, stderr) {
      if (!err) {
