...
class core {
  ...
  static composer(dir: string, type: "update" | "install" | "validate" | "upgrade" | "self-update") {
    if (type) {
      exec(`cd ${dir} && php libs/bin/composer/composer.phar ${type}`, (error: { message: any; }, stdout: any, stderr: any) => {
        if (error) {
            ...
            if (!err) {
              fs.writeFile(outputcss, result.css, function (err) {
                if (!err) {
                  ...
                  log(`${filename} > ${outputcss} ${chalk.green('success')}`);
    ...
    fs.exists(folder, function (exists) {
      if (exists && fs.lstatSync(folder).isDirectory()) {
        ...
        if (Array.isArray(read)) {
          ...
          js.unique().forEach(function (file: string) {
