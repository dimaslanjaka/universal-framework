...
gulp.task("build", function () {
  return gulp.src(srcCode).pipe(sourcemaps.init()).pipe(babel()).pipe(sourcemaps.write(".")).pipe(gulp.dest("dist"));
});

gulp.task("copy", function () {
  return gulp.src("./src/jsdocConfig.json").pipe(gulp.dest("dist"));
});
...
gulp.task(
  ...
  gulp.series([
    "lint",
    function watching() {
      return gulp.watch(lintedFiles, gulp.series(["lint"]));
    },
  ])
);
