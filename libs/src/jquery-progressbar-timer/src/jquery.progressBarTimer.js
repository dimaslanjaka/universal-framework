...
(function ($, window, document, undefined) {
  ...
  $.extend(Plugin.prototype, {
    init: function () {
      $(this).empty();
      var barContainer = $("<div>").addClass(
        "progress active progress-striped"
      );
      var bar = $("<div>")
        .addClass("progress-bar")
        .addClass(this.settings.baseStyle)
    ...
    addSeconds: function (seconds) {
      var added = this._getTicksFromSeconds(seconds);
      this.ticks += added;
    ...
    _draw: function () {
      ...
      if (this.settings.label && this.settings.label.show === true) {
        if (this.settings.label.type == "percent") {
          ...
          if (this.settings.smooth === true) {
            bar.html(
              Math.round(elapsedTicks / 50) + "/" + Math.round(this.ticks / 50)
            );
          } else {
      ...
      --this.remainingTicks;
      if (
        this._getSecondsFromTicks(this.remainingTicks) <=
          this.settings.warningThreshold &&
        !bar.hasClass(this.settings.warningStyle)
      ) {
        bar
          .removeClass(this.settings.baseStyle)
          .addClass(this.settings.warningStyle);
      }
