...
class PHPMailer
{
  ...
  {
    $this->exceptions = (true == $exceptions);
  }
  ...
  protected function MailSend($header, $body)
  {
    ...
    if ('' != $this->Sender && strlen(ini_get('safe_mode')) < 1) {
      ...
      if (true === $this->SingleTo && count($toArr) > 1) {
  ...
  public function SetLanguage($langcode = 'en', $lang_path = 'language/')
  {
    ...
    return true == $l; //Returns false if language not found
  }
    ...
    while (!$foundSplitPos) {
      ...
      if (false !== $encodedCharPos) {
        ...
        if ($dec < 128) { // Single byte character.
          ...
          $maxLength = (0 == $encodedCharPos) ? $maxLength :
  ...
  public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream')
  {
    try {
      ...
      $e->getMessage() . "\n";
      if (self::STOP_CRITICAL == $e->getCode()) {
        return false;
      }
    }
  ...
  public function EncodeQPphp($input = '', $line_max = 76, $space_conv = false)
  {
    ...
    while (list(, $line) = each($lines)) {
      ...
      for ($i = 0; $i < $linlen; ++$i) {
        $c = substr($line, $i, 1);
        $dec = ord($c);
        if ((0 == $i) && (46 == $dec)) { // convert first point in the line into =2E
          $c = '=2E';
        }
        if (32 == $dec) {
