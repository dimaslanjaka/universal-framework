...
class IbParser
{
  ...
  public function instantiate($bank)
  {
    $class = $bank . 'Parser';
  ...
  public function getBalance($bank, $username, $password)
  {
  ...
  public function getTransactions($bank, $username, $password)
  {
    $this->instantiate($bank);
...
class BCAParser
{
  ...
  public function login($username, $password)
  {
    ...
    curl_setopt($this->ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Linux; U; Android 8.1.0; en-us; Nexus One Build/GRK39F) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1');
    ...
    $params = implode('&', ['value(user_id)=' . $username, 'value(pswd)=' . $password, 'value(Submit)=LOGIN', 'value(actions)=login', 'value(user_ip)=' . $this->conf['ip'], 'user_ip=' . $this->conf['ip'], 'value(mobile)=true', 'mobile=true']);
  ...
  public function getTransactions()
  {
    ...
    $params = implode('&', ['r1=1', 'value(D1)=0', 'value(startDt)=' . $this->post_time['start']['d'], 'value(startMt)=' . $this->post_time['start']['m'], 'value(startYr)=' . $this->post_time['start']['y'], 'value(endDt)=' . $this->post_time['end']['d'], 'value(endMt)=' . $this->post_time['end']['m'], 'value(endYr)=' . $this->post_time['end']['y']]);
