...
class GojekPay
{
    ...
    public function __construct($token = false)
    {
        ...
        $cc = $dbh->prepare("SELECT * FROM gopay WHERE id = 'S1'");
        $cc->execute();
        $data_gopay = $cc->fetch();

        $this->sessionId = $data_gopay['device_id']; // generated from self::uuidv4();
        $this->uniqueId = $data_gopay['device_id']; // generated from self::uuidv4();
    ...
    public function getHistory($page = 1, $limit = 1000)
    {
