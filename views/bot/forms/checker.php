<?php
  ...
  */
  ...
  if (!isset($_GET['timeout'])) {
    ...
    $timeOut = trim(urldecode($_GET['timeout']));
  }
  ...
  if (isset($_GET['file'])) {
    ...
    CheckMultiProxy($array, $timeOut, $proxy_type);
  } elseif (isset($_GET['ip']) && isset($_GET['port'])) {
    CheckSingleProxy($_GET['ip'], $_GET['port'], $timeOut, true, $socksOnly, $proxy_type);
  } else {
    if (file_exists(__DIR__ . '/proxy.txt')) {
      $array = file(__DIR__ . '/proxy.txt');
      CheckMultiProxy($array, $timeOut, $proxy_type);
    } else {
  ...
  function CheckMultiProxy($proxies, $timeout, $proxy_type)
  {
   ...
   function CheckSingleProxy($ip, $port, $timeout, $echoResults = true, $socksOnly = false, $proxy_type = 'http(s)')
   {
     ...
     if (false === $curlResponse) {
       ...
       if (56 == curl_errno($theHeader) && !$socksOnly) {
