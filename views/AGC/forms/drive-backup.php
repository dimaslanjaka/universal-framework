...
$tokenfile = wpgoogle::tokenfile_gen2();
...
if (isset($_GET['code']) && isset($_GET['state'])) {
  if (strval($_SESSION['state']) == strval($_GET['state'])) {
    ...
    file_put_contents($tokenfile, json_encode($gClient->getAccessToken()), true);
    $gClient->setAccessToken($gClient->getAccessToken());
  } else {
    exit('Session State Doesnt Pair');
  }
}
...
if ($gClient->getAccessToken()) {
  ...
  if ($gClient->isAccessTokenExpired()) {
    ...
    $_SESSION['token'] = $gClient->getAccessToken();
    file_put_contents($tokenfile, json_encode($gClient->getAccessToken()));
  }
