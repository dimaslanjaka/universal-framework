<?php 
/*
Plugin Name:WP AMT Spinner
Plugin URI: http://wpamt.com
Description: Spin artikel secara otomatis untuk menghasilkan artikel baru yang lebih fresh .
Version: 1.0.0
Author: BeritahuPress
Author URI: http://beritahupress.com
*/ 

/*  Copyright 2019  WP AMT Spinner (email : kangamet01@gmail.com) */
...
function wp_auto_spinner_create_meta_box() {
	...
	foreach ($wp_spinner_types as $type){
		...
		add_meta_box( 'wp_auto_spinner-meta-boxes', 'WP AMT Spinner ', 'wp_auto_spinner_meta_boxes', $type , 'normal', 'high' );
...
function wp_auto_spinner_admin_head() {
         ...
         echo '<script src="'.plugins_url('js/main.wpautospinner.js?v=1.4.0', __FILE__).'" type="text/javascript"></script>';
}
...
function wp_auto_spinner_control_menu() {
 ...
 add_menu_page( 'WP spinner Synonyms', 'WP AMT Spinner', 'administrator', 'wp_auto_spinner', 'wp_auto_spinner_synonyms_fn', 'dashicons-wp-spinner-pencil-square-o', 77777787777777 );
	
	
	
  $synonymsSlug=add_submenu_page( 'wp_auto_spinner',  'Wp WP AMT Spinner synonyms','Kamus', 'administrator', 'wp_auto_spinner', 'wp_auto_spinner_synonyms_fn' );
  add_action('admin_head-'.$synonymsSlug, 'wp_auto_spinner_admin_head_synonyms');
 
  $synonymsSlug=add_submenu_page( 'wp_auto_spinner', 'Wp WP AMT Spinner custom thesaurus', 'Kamus Ku', 'administrator', 'wp_auto_spinner_thesaurus', 'wp_auto_spinner_thesaurus' );
  add_action('admin_head-'.$synonymsSlug, 'wp_auto_spinner_admin_head_thesaurus');
  
  add_submenu_page( 'wp_auto_spinner', 'Wp WP AMT Spinner settings', 'Pengaturan', 'administrator', 'wp_auto_spinner_settings', 'wp_auto_spinner_fn' );
  
  $logSlug = add_submenu_page( 'wp_auto_spinner', 'Wp WP AMT Spinner Log', 'Log', 'administrator', 'wp_auto_spinner_log', 'wp_auto_spinner_log' );
  add_action('admin_head-'.$logSlug, 'wp_auto_spinner_admin_head_log');
  
  $queueSlug=add_submenu_page( 'wp_auto_spinner', 'Wp WP AMT Spinner Spinning Antrian', 'Panduan', 'administrator', 'wp_auto_spinner_queue', 'wp_auto_spinner_queue' );
...
function wp_auto_spinner_publish($post_id){
	...
	if ( isset($_SERVER['HTTP_REFERER']) && stristr($_SERVER['HTTP_REFERER'], 'edit.php') )  return  ;
