...
class Employee_Information_Form extends Predefined_Form
{
    ...
    public function __construct()
    {
        ...
        $this->emails = [
            ["fromName" => "{first-name} {last-name}", "fromEmail" => "{email}", "toEmail" => "{admin_email}", "replyTo" => "", "ccEmail" => "", "bccEmail" => "", "emailSubject" => "A new employee information was submitted", "emailAttachment" => "", "emailBody" => "<p>Applicant information</p><p>{imagepreview:{photo}}</p><p>{first-name} {last-name} ({email})</p><p>{date-of-birth}</p><p>{phone}</p><p>Address:</p><p>{city}, {state}</p><p>{address1}</p><p>{address2}</p><p>Department: {department}</p><p>Position: {position}</p>", "index" => "0"],
        ];
