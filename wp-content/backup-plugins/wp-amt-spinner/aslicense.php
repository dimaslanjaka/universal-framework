...
function wp_auto_spinner_license_notice() {
	...
	if(!function_exists('curl_init')){
		echo '<div class="error"><p>You do not have <a href="http://curl.haxx.se/">Curl library</a> installed. you must install it for "WP AMT Spinner" plugin to function.  </p></div>';
		return;
	} 
	...
	if(trim($purchase) == '' ){
		if( ! stristr ($_SERVER['REQUEST_URI'] ,'spinner_settings' ) ){
			echo '<div class="updated"><p>WP AMT Spinner is ready please <a href="'.admin_url('admin.php?page=wp_auto_spinner_settings').'">click here</a> to add your purchase code and activate the plugin now </p></div>';
		}
	}else{
		...
		if(trim($licenseactive) == 'active' ){
			...
			if($minutes_diff > 1440 ){
				...
				//reset date
				update_option('wp_auto_spinner_license_active_date', time());
				//reactivate
				//activating
				//curl ini
				$ch = curl_init();
				...
				//curl get
				$x='error';
				$url='http://wpamt.com/license/index.php?itm=4092452&domain='.$_SERVER['HTTP_HOST'].'&purchase='.trim($purchase);
			...
			if( ! stristr ($_SERVER['REQUEST_URI'] ,'wp_auto_spinner_settings' ) ){
				echo '<div class="updated"><p>WP AMT Spinner is not active please <a href="'.admin_url('admin.php?page=wp_auto_spinner_settings').'">click here</a> to revise your purchase code and activate the plugin now </p></div>';
			}
