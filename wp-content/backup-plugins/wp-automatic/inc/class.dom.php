...
class wpAutomaticDom {
	...
	function getSimilarLinks($xpath){
		...
		$xpath = trim($xpath);
		if(! stristr($xpath, '/a/') &&  ! preg_match('{/a$}',$xpath) ){
			...
			if(!preg_match('{/a$}', $xpath) && stristr($xpath, '/a/')){
			...
			print_r($chosenNodePathParts);
 
			
			echo '<br>chosenNodePath:'.$chosenNodePath.'<br>';
			...
			foreach ($allLinksMatches as $singleLink){
				...
				if($currentNodePathNoDigits == $chosenNodePathNoDigits){
					...
					$LinksWithSimilarPath[] = $singleLink->getAttribute('href').'<br><br>'.$singleLink->getNodePath().'<br><br>';
			...
			if(count($LinksWithSimilarPathStrict) > 5){
				...
				print_r($values);
				print_r($LinksWithSimilarPath);
				
				return $LinksWithSimilarPathStrict ;
			}else{
				return($LinksWithSimilarPath);
			}
