...
require __DIR__ . '/YouTube.class.php';
require __DIR__ . '/YouTube.channel.php';
// YouTube is capitalized twice because that's how youtube itself does it:
// https://developers.google.com/youtube/v3/code_samples/php
class YouTubeDownloader2 extends YouTube
{
  private $storage_dir;
  private $cookie_dir;
  ...
  public function stream($id)
  {
    ...
    curl_setopt($ch, CURLOPT_HEADERFUNCTION, function ($ch, $data) use (&$headers, &$headers_sent) {
      ...
      if (preg_match('@HTTP\/\d\.\d\s(\d+)@', $data, $matches)) {
        ...
        if ($headers_sent && 2 == count($parts) && in_array(trim(strtolower($parts[0])), $forward)) {
          header(rtrim($data));
        }
      }
